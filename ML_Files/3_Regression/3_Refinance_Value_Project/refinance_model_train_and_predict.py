...
if __name__ == "__main__":
    ...
    try:
        # Training
        X, y = load_data('Weekly_Refinance_Volumes_Data.xlsx', target_column='Refinance', drop_columns=drop_columns)
        model, scaler = train_rf_model(X, y, params={'n_estimators': 100, 'max_depth': 5})
        save_model(model, 'RF_model.sav')
        save_scaler(scaler, 'scaler.sav')
    except Exception as e:
    ...
    try:
        ...
        future_data_full = pd.read_csv('future_data.csv')
        future_data_full = future_data_full.reset_index(drop=True)
        future_data_full['Prediction'] = predictions
        future_data_full.to_excel('predictions_on_future_data.xlsx', index=False)
        logging.info("Predictions saved to predictions_on_future_data.xlsx")
    except Exception as e:
